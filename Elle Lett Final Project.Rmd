...
Part 1: Simulate n=1,000,000 individuals
...
4. Fit the outcome model based on the simulated data and observed gender, and estiamte the observed association between the outcome and gender identity $\hat{\mathrm{OR}}$
...
In studies using claims data, the percentage of individuals meeting the diagnosis code-based criteria has been reported to be as low as 0.026% which suggests potentially very severe misclassification, given the order of magnitude difference between the observed transgender population in these national databases, and the estimated population frequency of transgender individuals. Therefore, we test a broad range of misclassification probabilities with the probability of misclassification ranging from 10%, 30%, 50%, and 70%. Lastly, we simulate bias under a wide range of estimated effect sizes, from 1.50 to 6.00, because prior studies have estimated effect sizes in transgender health inequities as high as 8.5, comparing the odds of mental health diagnoses across gender identities.
...
betas<-rbind(c(-2.302,log(1.5)),c(-2.302,log(3.0)),c(-2.302,log(4.5)),c(-2.302,log(6)))
effect_scenarios<-data.frame(Strength=c("Mild","Moderate", "Strong","Severe"),betas,exp(betas[,2]))
...
alphas<-rbind(c(-2.302,0),c(-2.302,1.0995),c(-2.302,1.6095),c(-2.302,1.946))
miss_scenarios<-data.frame(Misclassification=c("Mild","Moderate", "Strong","Severe"),alphas,apply(alphas,1,function(x){round(exp(sum(x)),2)}))
...
Below is the function to simulate the data for a given scenario, store the estimates ($\hat{\mathrm{OR}}$) for each simulated data set, as well as the delta method standard errors. Also, throughout the code below we use matrix multiplication to obtain linear combinatios of the simulated variables and parameters to improve computational efficienty and reduce the need for loops.
```{r simulation_function}

sim<-function(j,k,N=10^3,n=10^3,pi_G=0.10,Betas=betas,Alphas=alphas){ 
...
key<-c() #key will be used to identify which scenario is in which list
for(j in 1:4){
  for(k in 1:4){
